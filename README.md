...
<details>
  ...
  Optional (spline lamella geometry in *PACEtomo_measureGeometry.py* / `geometryMode = "spline"`):
  - [PyTorch](https://pytorch.org/get-started/locally/) (CPU is sufficient)
  - [torch-cubic-spline-grids](https://github.com/teamtomo/teamtomo/tree/main/packages/primitives/torch-cubic-spline-grids) (`pip install torch-cubic-spline-grids`)
</details>
...
| `zeroExpTime` | `0` | Custom exposure time [s] for the first tilt image (`zeroExposure` in some docs). This can be useful for hybrid processing approaches. When set to `0` the same exposure time will be used for all tilt angles. When `typress` is `True`, this applies only to the extra start-tilt image, not the tilt series. |
| `zeroDefocus` | `0` | Custom target defocus for the first tilt image. When set to `0` the same target defocus will be used for all tilt angles. When `typress` is `True`, this applies only to the extra start-tilt image, not the tilt series. |
| `typress` | `False` | If `True`, each target at `startTilt` is recorded twice: first with `zeroExpTime` / `zeroDefocus` / `zeroMag`, then a normal Record for the tilt series. The extra image is saved as `{curDir}/typress/{stem}_typress.mrc`. Camera frames go to a `typress` folder via `SetFolderForFrames` (restored after the extra shot). Skipped on recover. |
| `zeroMag` | `0` | Nominal magnification for the extra `typress` image. `0` keeps Record mag. Ignored unless `typress` is `True` so the tilt series does not mix magnifications. |
...
| `geometryMode` | `"plane"` | `"plane"` uses only pretilt/rotation for target height `z0`. `"spline"` looks up `z(SSX, SSY)` from `geometry_file` (plane plus cubic B-spline residual from *PACEtomo_measureGeometry.py*), so a wavy or deformed lamella is not forced to a single plane. Pretilt/rotation from the file are still used for `autoStartTilt` and tilt scaling. |
| `geometry_file` | `""` | Path to the JSON written by *PACEtomo_measureGeometry.py*. Required when `geometryMode = "spline"`. |
...
This script measures z-height at navigator geo points, fits a plane (*pretilt* and *rotation*), then a cubic B-spline residual so the specimen can be wavy rather than a single plane. Set PACEtomo `geometryMode = "spline"` and `geometry_file` to the written JSON to use `z(SSX, SSY)` for each target’s focus. Requires `torch` and `torch-cubic-spline-grids` (CPU is enough).
- How to use:
  - Add navigator points on a montage or low-mag image (**Caution:** do not expose areas you want to image later).
  - Put their indices in `geo_nav_item_list` (at least 3). The **first** point is the specimen origin (`SSX=SSY=0`).
  - Choose `geo_defocus_method`: `beam_tilt` / `beam_tilt_sem` / `ctf`, or `beam_tilt_then_ctf` (default) to measure with beam-tilt then refine with CtfFind after bringing defocus into range.
  - `spline_resolution` defaults to `(3, 3)` control points. Use more points (and a finer grid) if the lamella is strongly deformed.
  - Copy the script into a SerialEM slot and run it. It writes CSV, JSON, and a 3D plot, then restores the origin nav item.
