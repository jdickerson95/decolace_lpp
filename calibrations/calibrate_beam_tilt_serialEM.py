#!Python
# ===================================================================
# ScriptName     calibrate_beam_tilt_serialEM
# Purpose:       Sweep defocus and fit the systematic offset between
#                SerialEM sem.G(-1) autofocus defocus and CTF:
#                  delta = measured - ctf = intercept + slope * defocus
#                Writes beam_tilt_serialEM_calibration.json for
#                PACEtomo_beamTiltDefocus.calibration_file with
#                defocusMethod = beam_tilt_sem in PACEtomo.
# ===================================================================
import sys
...
# X-tilt during sem.G(-1) measurement (match PACEtomo beam_tilt_xtilt_*).
serialEM_xtilt_x = 0.0
serialEM_xtilt_y = 0.0
...
csv_measurements = "beam_tilt_serialEM_measurements.csv"
calibration_json = "beam_tilt_serialEM_calibration.json"
plot_file = "beam_tilt_serialEM_fits.png"
...
def measure_serialEM_defocus():
    xtilt = sem.ReportXLensDeflector(xtilt_lens_index)
    try:
        set_xtilt(serialEM_xtilt_x, serialEM_xtilt_y)
        sem.G(-1)
        measured_um = float(sem.ReportAutoFocus()[0])
        return {"serialEM_defocus_um": measured_um}
    finally:
        set_xtilt(float(xtilt[0]), float(xtilt[1]))
...
def make_plot(path, rows, delta_fit):
    ...
    measured = np.array([float(r["serialEM_defocus_um"]) for r in rows], dtype=float)
    ...
    ax = axes[0]
    ax.scatter(ctf, measured, label="sem.G(-1)")
    ...
    ax.set_ylabel("SerialEM defocus (um)")
    ax.set_title("sem.G(-1) measured and corrected vs CTF")
...
def main():
    ...
    sem_xtilt_label = (
        f"sem.G(-1) X-tilt ({serialEM_xtilt_x:.6f}, {serialEM_xtilt_y:.6f})"
    )
    echo("##### SerialEM G(-1) defocus scaling calibration #####")
    ...
    try:
        for target_defocus in target_defocus_values:
            ...
            measure = measure_serialEM_defocus()
            ctf_defocus, ctf_res = acquire_ctf_reference()

            row = {
                "target_defocus_um": float(target_defocus),
                "ctf_defocus_um": float(ctf_defocus),
                "serialEM_defocus_um": measure["serialEM_defocus_um"],
                "delta_um": measure["serialEM_defocus_um"] - ctf_defocus,
                "ctf_resolution_A": float(ctf_res),
            }
            rows.append(row)
            echo(
                f"sem.G(-1)={measure['serialEM_defocus_um']:.4f} um, "
        ...
        calibration = {
            "model": "beam_tilt_serialEM_scaling",
            "measurement_method": "serialEM_G(-1)",
            "created": datetime.now().isoformat(timespec="seconds"),
            "ctf_xtilt_x": float(ctf_xtilt_x),
            "ctf_xtilt_y": float(ctf_xtilt_y),
            "serialEM_xtilt_x": float(serialEM_xtilt_x),
            "serialEM_xtilt_y": float(serialEM_xtilt_y),
            "delta_offset": {
        ...
        echo("  Set PACEtomo defocusMethod = beam_tilt_sem")
        echo("================================================")
